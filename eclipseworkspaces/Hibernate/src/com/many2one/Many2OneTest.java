...
public class Many2OneTest {
	...
	public void test3() throws Exception {
			 Session session = HibernateUtil.getSession();
			 String hql = "select count(e) from Employee e where e.name like :hh";
			 Long totalCount = (Long)session.createQuery(hql).setParameter("hh", "%a%").uniqueResult();
			 System.out.println(totalCount);
			 session.close();
	}
	@Test
	public void test4() throws Exception {
		Session session = HibernateUtil.getSession();
		String sql = "select e from Employee e where e.id in (:ida)";
		List<Employee> list = session.createQuery(sql).setParameterList("ida", new Long[] {new Long("1"),new Long("2"),new Long("3"),}).list();
